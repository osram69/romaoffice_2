...
import { buildRequestPdf, requestPdfFilename } from "@/lib/pdf";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const attachments = [{ filename: requestPdfFilename(product.code, data.lang, order.publicId, { signed: true }), content: pdf, contentType: "application/pdf" }];
    const letter = it
    ...
    return new NextResponse(new Uint8Array(pdf), { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${requestPdfFilename(product.code, data.lang, order.publicId, { signed: true })}"`, "Cache-Control": "no-store" } });
  } catch (error) {
