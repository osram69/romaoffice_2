...
const input = z.object({ orderId: z.string().uuid(), provider: z.enum(["stripe", "paypal", "sumup"]), testToken: z.string().optional() });
export async function POST(req: NextRequest) {
  try {
    ...
    return json(await checkoutOrder(req, parsed.data.orderId, parsed.data.provider, parsed.data.testToken));
