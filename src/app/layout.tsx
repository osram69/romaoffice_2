...
export default async function RootLayout({ children }: { children: ReactNode }) {
  ...
  if (pathname.startsWith("/gestione-tariffe-x9k2m7") || pathname.startsWith("/gestione-domiciliazioni-x9k2m7")) return <html lang="it" className={jost.variable}><body>{children}</body></html>;
  ...
  return <html lang={lang} className={jost.variable}><body><Analytics /><Header lang={lang} alternate={alternateFor(key)} />{children}<Footer lang={lang}/><CookieConsent lang={lang}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(jsonLd).replace(/</g,"\\u003c")}} /></body></html>;
}
