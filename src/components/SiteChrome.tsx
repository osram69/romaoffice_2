...
export function Footer({ lang: initialLang }: { lang: Lang }) {
  ...
  return <footer className="site-footer"><div className="footer-grid shell">
    ...
    <div><h2>{t.hours}</h2><p>{t.center}<br />{t.weekdays}<br />{t.saturday}</p><Link href={it ? "/dove-siamo.html" : "/en/where-we-are.html"}>{it ? "Dove siamo" : "Find us"}</Link><br /><Link href={it ? "/gallery.html" : "/en/gallery.html"}>Gallery</Link><br /><Link href={it ? "/area-clienti.html" : "/en/customer-area.html"}>{t.customer}</Link></div>
