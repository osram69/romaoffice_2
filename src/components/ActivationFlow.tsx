    ...
    <div className="activation-grid">
      <div className="activation-form">
        {step === 1 && <form noValidate onSubmit={e => { e.preventDefault(); sendCode(); }}>
          ...
          {product.addons.some(addon => addon.selectable) && <fieldset className="postal-addons"><legend>{it ? "Servizi opzionali — Allegato 1" : "Optional services — Annex 1"}</legend><p className="form-note">{it ? "I canoni selezionati si sommano al prezzo della domiciliazione. I servizi a consumo saranno addebitati solo se richiesti." : "Selected fees are added to the address-service price. Usage-based services are charged only when requested."}</p>{product.addons.filter(addon => addon.selectable).map(addon => {
