...
export function CookiePolicyContent({ lang }: { lang: Lang }) {
  ...
  return <>
    ...
    <h3>{it ? "Chat del sito (tawk.to)" : "Site chat (tawk.to)"}</h3>
    <p>{it ? "Usiamo tawk.to per la chat di assistenza in basso a destra. Lo script si carica su ogni pagina, indipendentemente dal consenso cookie, perché è lo strumento con cui offriamo assistenza in tempo reale: tawk.to imposta propri cookie per riconoscerti tra una pagina e l’altra e mantenere la conversazione aperta. Non li usiamo per pubblicità." : "We use tawk.to for the support chat in the bottom-right corner. The script loads on every page, regardless of cookie consent, because it is the tool we use to offer real-time support: tawk.to sets its own cookies to recognise you across pages and keep the conversation open. We do not use them for advertising."}</p>
