...
export function StandardOfferModal({ service, lang }: { service: ServiceCode; lang: Lang }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const emailErr = validateEmailField(String(fd.get("email") || "")); if (emailErr) { setError(emailErr); return; }
  ...
  return <>
    ...
    <dialog ref={dialog} className="standard-offer-dialog" aria-labelledby={id} onCancel={close}>
      ...
      {state === "sent" ? <div className="standard-offer-success" role="status"><CheckCircle2 /><h3>{it ? "Offerta inviata" : "Offer sent"}</h3><p>{it ? "Abbiamo inviato l’offerta standard e il modulo PDF originale a" : "We sent the standard offer and original PDF form to"} <strong>{email}</strong>.</p><p>{it ? "Controlla anche la cartella spam." : "Please also check your spam folder."}</p><button className="button primary" onClick={close}>{it ? "Chiudi" : "Close"}</button></div> : <form onSubmit={submit} onChange={() => { if (state !== "sending") setRequestId(crypto.randomUUID()); }} noValidate className="standard-offer-form"><p className="form-note">{it ? "Riceverai prezzi, condizioni e il modulo di richiesta PDF originale. Nessun acquisto viene effettuato con questa richiesta." : "You will receive prices, terms and the original PDF application form. This request does not make a purchase."}</p><div className="form-grid"><label htmlFor={`${id}-first`}>{it ? "Nome" : "First name"}*<input id={`${id}-first`} ref={first} name="firstName" autoComplete="given-name" maxLength={100} required /></label><label htmlFor={`${id}-last`}>{it ? "Cognome" : "Last name"}*<input id={`${id}-last`} name="lastName" autoComplete="family-name" maxLength={100} required /></label><label htmlFor={`${id}-title`}>{it ? "Titolo" : "Title"}*<select id={`${id}-title`} name="title" required defaultValue=""><option value="">{it ? "Seleziona" : "Select"}</option><option value="Mr">Mr</option><option value="Ms">Ms</option></select></label><label htmlFor={`${id}-email`}>Email*<input id={`${id}-email`} name="email" type="email" autoComplete="email" maxLength={254} required onBlur={e => { const msg = validateEmailField(e.target.value); const invalidEmailMsg = it ? "Inserisci un indirizzo email valido." : "Enter a valid email address."; setError(prev => msg || (prev === invalidEmailMsg ? "" : prev)); }} /></label></div><label htmlFor={`${id}-notes`}>{it ? "Richieste o messaggio (facoltativo)" : "Requests or message (optional)"}<textarea id={`${id}-notes`} name="notes" maxLength={1000} rows={3} placeholder={it ? "Es. durata desiderata, esigenze particolari, domande…" : "E.g. preferred duration, specific needs, questions…"} /></label><div className="honeypot" aria-hidden="true"><input name="website" aria-label="Website" tabIndex={-1} autoComplete="off" /></div><div className="consent-box"><label className="check-label"><input type="checkbox" name="consent" required /><span>{it ? "Acconsento al trattamento dei dati personali per ricevere l’offerta richiesta." : "I consent to the processing of my personal data to receive the requested offer."} <LegalLinkModal className="consent-link" label={it ? "Informativa privacy" : "Privacy notice"} title={it ? "Informativa Privacy" : "Privacy Notice"}><PrivacyNoticeContent lang={lang}/></LegalLinkModal>*</span></label></div><div className="field-error" role="alert">{error}</div><button className="button primary" type="submit" disabled={state === "sending"}>{state === "sending" ? <LoaderCircle className="spin" /> : <ArrowRight />}{it ? (state === "sending" ? "Invio in corso…" : "Continua") : (state === "sending" ? "Sending…" : "Continue")}</button></form>}
    </dialog>
  </>;
}
