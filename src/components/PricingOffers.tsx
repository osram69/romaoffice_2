...
import { ArrowRight, BadgePercent, Building2, FileDown, Mail } from "lucide-react";
...
export function PricingOffer({ product, lang }: { product: ProductOffer; lang: Lang }) {
  ...
  return <article className={`offer-card ${postal ? "postal-offer" : "legal-offer"}`} id={`offer-${product.code}`}>
    <div className="offer-head"><div><span className="offer-badge">{postal ? <Mail /> : <Building2 />}{it ? "ATTIVABILE ONLINE" : "AVAILABLE ONLINE"}</span>{product.onlineDiscountEnabled && <span className="offer-badge online-discount-badge"><BadgePercent/>{it ? `-${product.onlineDiscountBps / 100}% SOLO ONLINE` : `-${product.onlineDiscountBps / 100}% ONLINE ONLY`}</span>}<h3>{copy.name}</h3><p>{copy.description}</p></div><div className="offer-actions"><Link className="button primary" href={activationHref(product.code, lang)}>{it ? "Attiva Online" : "Activate Online"}<ArrowRight /></Link><ManualRequestModal product={product} lang={lang} /><StandardOfferModal service={product.code} lang={lang} /><a className="button secondary" href={`/${REQUEST_MODULE_FILENAME}`} download>{it ? "Scarica il modulo" : "Download the form"}<FileDown /></a></div></div>
    <div className="table-wrap"><table className="price-table"><caption className="sr-only">{copy.name} — {it ? "prezzi IVA esclusa" : "prices excluding VAT"}</caption><thead><tr><th scope="col">{copy.duration}</th><th scope="col">{copy.rate}</th><th scope="col">{postal ? (it ? "Attuale offerta" : "Current offer") : copy.offer}</th></tr></thead><tbody>{product.tiers.map(tier => <tr key={tier.months} className={active && tier.offerCents !== null ? "highlight" : ""}><th scope="row">{copy.months(tier.months)}</th><td>{active && tier.offerCents !== null ? <s className="price-struck">{formatEur(tier.listCents, lang)} <small>{it ? "+ IVA" : "+ VAT"}</small></s> : <>{formatEur(tier.listCents, lang)} <small>{it ? "+ IVA" : "+ VAT"}</small><small className="monthly-equivalent">≈ {monthlyEquivalent(tier.listCents, tier.months, lang)}/{it ? "mese" : "mo"}</small></>}</td><td>{tier.offerCents !== null ? <><b>{formatEur(tier.offerCents, lang)} <small>{it ? "+ IVA" : "+ VAT"}</small></b>{tier.newActivation && active && <span className="offer-flag"> (*)</span>}<small className="monthly-equivalent">≈ {monthlyEquivalent(tier.offerCents, tier.months, lang)}/{it ? "mese" : "mo"}</small></> : <span>—</span>}</td></tr>)}</tbody></table></div>
    ...
    {!postal && (product.smart3x24Active || product.smart6x24Active) && <SmartStartCard lang={lang} product={product} />}
...
function SmartStartCard({ lang, product }: { lang: Lang; product: ProductOffer }) {
  const it = lang === "it";
  return <div className="smart-start-card">
    ...
    <div className="smart-start-tiers">
      {product.smart3x24Active && <div><b>Smart-Start 3+24</b><p>{it ? "Prima tranche: 3 mesi a 20 €/mese (60 € + IVA) — poi 24 mesi a 1.000 € + IVA" : "First instalment: 3 months at €20/month (€60 + VAT) — then 24 months at €1,000 + VAT"}</p></div>}
      {product.smart6x24Active && <div><b>Smart-Start 6+24</b><p>{it ? "Prima tranche: 6 mesi a 20 €/mese (120 € + IVA) — poi 24 mesi a 1.080 € + IVA" : "First instalment: 6 months at €20/month (€120 + VAT) — then 24 months at €1,080 + VAT"}</p></div>}
    </div>
