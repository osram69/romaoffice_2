...
export function ManualRequestModal({ product, lang }: { product: ProductOffer; lang: Lang }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const phone = normalizePhone(String(fd.get("representativePhone") || ""));
    if (!/^\+[1-9]\d{7,14}$/.test(phone)) nextFieldErrors.representativePhone = it ? "Inserisci un cellulare valido con prefisso internazionale, ad esempio +39." : "Enter a valid mobile number with an international prefix, for example +39.";
    if (!isValidTaxCode(String(fd.get("representativeTaxCode") || ""))) nextFieldErrors.representativeTaxCode = it ? "Codice Fiscale non valido." : "Invalid tax code.";
    const startDate = String(fd.get("startDate") || "");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate) || startDate < today) nextFieldErrors.startDate = it ? "Inserisci una data valida, non nel passato." : "Enter a valid date, not in the past.";
    ...
    try {
      ...
      setEmail(String(fd.get("email"))); setState("sent"); setFieldErrors({}); form.reset();
    } catch (e) {
  ...
  return <>
    <button type="button" ref={trigger} className="button secondary manual-request-trigger" onClick={() => { if (state !== "sending") { setState("idle"); setError(""); setFieldErrors({}); } setOpen(true); }}><ClipboardList aria-hidden="true" />{it ? "Compila il modulo online" : "Fill in the online form"}</button>
