...
export function ManualRequestModal({ product, lang }: { product: ProductOffer; lang: Lang }) {
  ...
  return <>
    <button type="button" ref={trigger} className="button ghost manual-request-trigger" onClick={() => { if (state !== "sending") { setState("idle"); setError(""); } setOpen(true); }}><ClipboardList aria-hidden="true" />{it ? "Compila il modulo online" : "Fill in the online form"}</button>
